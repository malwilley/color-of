...
*/
function matchTermToColor(termColors, palette) {
  // map to rgb for use with color-diff library
  const paletteRgb = palette.map(util.colorToRgb);

  // convert each term color to the closest palette color
  const termPaletteColors = termColors
    .map(util.colorToRgb)
    .map(c => diff.closest(c, paletteRgb))
    .map(util.rgbToColor);
