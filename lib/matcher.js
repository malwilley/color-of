...
function matchTermToColor(termColors, colorOptions) {
  const palette = colorOptions.map(c => c.hex());
  const classifier = new ColorClassifier(palette);

  // match each of the term's color results to the color it is closest to
  const closestColorOptions = termColors
    .map(termColor => classifier.classify(termColor.hex()))
    .map(c => Color(c));
